...
wget -qO- https://launchpad.net/mg5amcnlo/lts/2.9.x/+download/MG5_aMC_v2.9.21.tar.gz | tar zxv
cd MG5_aMC_v2_9_17
sed -i '/import os/aos.unsetenv("PYTHIA8DATA")' bin/mg5_aMC
sed -i 's/6500/3500/g' ./madgraph/various/banner.py
printf "install lhapdf6\ninstall pythia8\nset auto_update 0" > mg_install.txt
bin/mg5_aMC -f mg_install.txt
rm mg_install.txt
cd -
...
cd Delphes-3.5.0
sed -i '/Emit/d' display/DelphesPlotSummary.cc
make -j 4
make -j 4 display
cd -
